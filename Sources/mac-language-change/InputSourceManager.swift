...
class InputSourceManager: ObservableObject {
    ...
    func refreshInputSources() {
        ...
        for index in 0..<count {
            ...
            guard let localizedNamePtr = TISGetInputSourceProperty(inputSource, kTISPropertyLocalizedName) else { continue }
    ...
    func switchToNextInputSource() {
        ...
        for index in 0..<count {
            ...
            // 활성화된 입력 소스인지 확인
            guard let enabledPtr = TISGetInputSourceProperty(inputSource, kTISPropertyInputSourceIsEnabled) else { continue }
        ...
        if let currentSourceNamePtr = TISGetInputSourceProperty(currentSource, kTISPropertyLocalizedName) {
            ...
            print("현재 선택된 입력 소스: \(currentSourceName) (ID: \(currentSourceID))")
        }
        ...
        print("nextInputSource: \(nextInputSource)")
        ...
        if status == noErr {
            ...
            while attempts < enabledSources.count - 1 {
                ...
                print("enabledSources.count: \(enabledSources.count)") 
                ...
                if tryStatus == noErr {
                    ...
                    if let currentSourceNamePtr = TISGetInputSourceProperty(tryInputSource, kTISPropertyLocalizedName) {
                        let currentSourceName = Unmanaged<CFString>.fromOpaque(currentSourceNamePtr).takeUnretainedValue() as String
                        print("최종 선택된 입력 소스: \(currentSourceName) (ID: \(currentSourceID))")
                    }

                    print("175 tryInputSource: \(tryInputSource)")
                    print("176 tryStatus: \(tryStatus)")
                    print("177 noErr: \(noErr)")
                    return
                }
                
                attempts += 1
            }
            print("enabledSources: \(enabledSources)")
